"""Clean Kaggle FP16 replication, shard B, for Stage 1b contrastive collection."""
...
EXPECTED_RESPONSES = 7_500

WORKING = Path("/kaggle/working")
PAYLOAD_ROOT = Path("/tmp/stage1b_payload")
OUTPUT = WORKING / "stage1b_contrastive_kaggle_fp16_shard_b.json"
REPORT = WORKING / "stage1b_kaggle_fp16_shard_b_runtime.json"
...
def patch_cloud_copy(root: Path) -> dict:
    ...
    oracle.write_text(oracle_text.replace(old_dtype, "dtype=torch.float16,"))
    ...
    collector_text = collector_text.replace(
        old_runtime,
        '"runtime": "pytorch_float16_cuda_sdpa_peft_kaggle_replication",',
    )
...
def main() -> None:
    ...
    runtime = {
        "schema_version": 1,
        "purpose": "noncanonical_clean_cuda_fp16_replication",
        "shard": SHARD,
    ...
    for alias in ENDPOINTS:
        command.extend(("--endpoint", alias))
    run(command, cwd=str(PAYLOAD_ROOT))
